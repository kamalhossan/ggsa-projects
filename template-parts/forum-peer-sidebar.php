...
$forum_query = new WP_Query( $forumArgs );
$total_topic = $forum_query -> found_posts;

// Get post IDs from the query results
$post_ids = wp_list_pluck($forum_query->posts, 'ID');
...
$taxonomy = 'topics';
$peer_terms = get_terms( array(
    'taxonomy'   => $taxonomy,
    'hide_empty' => true,
) );

$top_topics = [];
foreach($peer_terms as $term){

    $term_id = $term -> term_id;
    $term_count = $term -> count;
    $top_topics[$term_id] = $term_count;
...
<div class="peer-sidebar">
    <div class="topic-counter">
        <div class="d-flex rounded-2 border border-1 py-3 justify-content-center">
            <div class="topic p-3 border-end w-100">
                <h2 class="text-[#1B61F9] mb-1 p-0 fs-6">Topics</h2>
                <h4 class="fs-4 text-[#FAA332] fw-bold p-0 m-0"><?php echo $total_topic;?></h4>
            </div>
            <div class="discussion p-3 w-100">
                <h2 class="text-[#1B61F9] mb-1 p-0 fs-6">Discussions</h2>
    ...
    if($found_post){
        ...
        if($your_forums){
            $all_peer_forum_by_user = array_merge($user_post_ids, $your_forums);
        } else {
            $all_peer_forum_by_user = $user_post_ids; 
        }

    } else {
        $all_peer_forum_by_user = get_user_meta( get_current_user_id(), 'commented_post_ids', true);
        if(! $all_peer_forum_by_user){
            $all_peer_forum_by_user = array(0);
        }
    }
    ...
    $your_forum_args = array(
        'post_type'      => 'peer-forum',
        'posts_per_page' => -1,
        'post__in'       => $all_peer_forum_by_user,
    );
    ...
    <div class="top-contributors mt-3">
        <div class="rounded-3 border border-1 p-3">
            ...
            <div class="top-contributors-list mt-3">
                <div class="d-flex flex-row align-items-center" id="top-contributors" role="tablist">
                    <div style="color:#f6f6f6;" class="position-relative tab-image nav-link w-100 active" id="my-school-tab" data-bs-toggle="tab" href="#my-school" aria-controls="my-school" aria-selected="false" role="tab" tabindex="-1">
                    ...
                    <div style="color:#f6f6f6;" class="position-relative tab-image nav-link w-100" id="all-school-tab" data-bs-toggle="tab" href="#all-school" aria-controls="all-school" aria-selected="false" role="tab" tabindex="-1">
                        <svg style="height: 36px" class="w-100" viewBox="0 0 193 40" preserveAspectRatio="none" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                            <g filter="url(#filter0_d_6543_11707)">
                                <path d="M192 39H3C7.44526 31.4494 17.5052 14.7507 20.9343 9.47189C24.6131 3.80862 27.5968 3.00002 31.2756 3H160.73C167.352 3 169.927 5.83146 171.766 8.66292L192 39Z" fill="currentColor"></path>
                            </g>
                ...
                <div class="tab-content mt-3" id="top-contributors-content">
                    <div class="tab-pane fade active show" id="my-school" role="tabpanel" aria-labelledby="my-school-tab">
                        <?php my_school_top_contributors();?>
                    </div>
                    <div class="tab-pane fade" id="all-school" role="tabpanel" aria-labelledby="all-school-tab">
...
if(!empty($top_topics)){ ?>
    <div class="hot-topics mt-3">
        <div class="rounded-3 border border-1 p-3">
            <h4 class="fw-bold text-[#161C24]">Hot Topics</h4> 
            <div class="hot-topics-list">
                <?php
                    $topic_limit = 0;
                    foreach($top_topics as $term_id => $term_count){
                        $topic_limit++;
                        if($topic_limit >= 6){
                            break;
                        }
